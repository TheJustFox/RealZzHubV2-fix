...
local Themes =
	loadstring(game:HttpGet("https://raw.githubusercontent.com/RealZzHub/MainV2/main/Misc/themes.lua", true))()
...
function Ripple(Button) -- thanks xbox | edited a little for this ui lib | Hey buy falconss uwu
	...
	task.spawn(function()
		...
		NewCircle.Position =
			UDim2.new(0, zzMouse.X - NewCircle.AbsolutePosition.X, 0, zzMouse.Y - NewCircle.AbsolutePosition.Y)
		...
		NewCircle:TweenSizeAndPosition(
...
function Library:Main(GName)
	...
	function TabLibrary:Notify(NText, Time)
		...
		NotificationClose.InputBegan:Connect(function(input)
			if input.UserInputType == Enum.UserInputType.MouseButton1 then
				local tween = zzTweenService:Create(Notification, TweenInfo.new(0.25, Enum.EasingStyle.Sine), {
		...
		local tween1 = zzTweenService:Create(
			NotificationBar,
		...
		tween1.Completed:Connect(function()
			local tween2 = zzTweenService:Create(Notification, TweenInfo.new(0.25, Enum.EasingStyle.Sine), {
				Position = UDim2.new(1, 300, Notification.Position.Y.Scale, Notification.Position.Y.Offset),
			})
			tween2:Play()
	...
	function TabLibrary:NewTab(TName)
		...
		local BrokenToggles = {
			["No Roll/Slide Cooldown"] = true,
			["Roll As Infected"] = true,
			["No Firerate"] = true, --// Very fucking unsafe
			["Always Auto"] = true, --// Doesn't even work
		}

		function ItemLibrary:NewToggle(TGName, callback, State)
			if BrokenToggles[TGName] then
				return
			end
		...
		function ItemLibrary:NewSlider(SName, MinVal, MaxVal, Inc, callback, StartValue)
			...
			callback = callback or function() end
			if SName == "Auto Shoot All Delay" then
				print("[+] Spoofing Auto Shoot All Delay...")
				MinVal = 30
			end
			...
			SliderValue.FocusLost:Connect(function()
				if #SliderValue.Text > 0 then
					if tonumber(SliderValue.Text) then
						...
						Slider:TweenSize(
							UDim2.new((Val - MinVal) / (MaxVal - MinVal), 0, 1, 0),
							"Out",
							"Quad",
							0.05,
							true
					...
					set(
						MinVal
							+ (
								(MaxVal - MinVal)
								* math.clamp(
									(zzMouse.X - SliderBackground.AbsolutePosition.X) / SliderBackground.AbsoluteSize.X,
									0,
									1
								)
							)
		...
		function ItemLibrary:NewKeybind(KName, Key, callback)
			...
			zzUIS.InputBegan:connect(function(cur, pressed)
				if not pressed then
					if cur.KeyCode.Name == CurrentKey or cur.UserInputType.Name == CurrentKey then
						pcall(
							callback,
							((cur.UserInputType == Enum.UserInputType.Keyboard and cur.KeyCode) or cur.UserInputType)
						)
					end
				end
			end)
			...
			Keybind.MouseButton1Click:connect(function()
				...
				Keybind.Text = "[ "
					.. tostring(
						(i.UserInputType == Enum.UserInputType.Keyboard and i.KeyCode.Name) or i.UserInputType.Name
					)
					.. " ]"
				CurrentKey = tostring(
					(i.UserInputType == Enum.UserInputType.Keyboard and i.KeyCode.Name) or i.UserInputType.Name
				)
				Config[TName][KName] = (i.UserInputType == Enum.UserInputType.Keyboard and i.KeyCode.Name)
					or i.UserInputType.Name
				wait()
				IsFocused = false
			end)
		...
		function ItemLibrary:NewDropdown(DName, Items, callback, ShowItem, UseItem)
			...
			local DropdownContainerFrameUIListLayout = Instance.new("UIListLayout")
			...
			DropdownContainerFrame.Position = UDim2.new(0.0315457396, 0, 0.373443991, 0)
			...
			DropdownContainerFrameUIListLayout.SortOrder = Enum.SortOrder.LayoutOrder
			...
			local function closeOpen()
				if not Open and (#Items - 1) > 0 then
					...
					local tween =
						zzTweenService:Create(DropdownContainerFrame, TweenInfo.new(0.3, Enum.EasingStyle.Sine), {
							Size = UDim2.new(0, 297, 0, math.min((#Items * 28), 500)),
						})
					tween:Play()
					tween.Completed:Connect(function()
						for _, v in pairs(DropdownContainerFrame:GetChildren()) do
							if v ~= DropdownContainerFrameUIListLayout and v ~= DropdownContainerFrameUICorner then
								v.Visible = true
							end
						end
					end)
					local tween1 = zzTweenService:Create(
						DropdownToggle,
						TweenInfo.new(0.3, Enum.EasingStyle.Linear, Enum.EasingDirection.In, 0),
						{ Rotation = 90 }
					)
					tween1:Play()
				else
		...
		function ItemLibrary:NewColorpicker(CName, callback, Color)
			...
			ColorpickerToggleFrameUICorner.Name = "ColorpickerToggleFrameUICorner"
			...
			ColorpickerContainerFrameUICorner.Name = "ColorpickerContainerFrameUICorner"
			...
			local function set(h, s, v)
				...
				local tween1 = zzTweenService:Create(
					SatButton,
					TweenInfo.new(0.05, Enum.EasingStyle.Sine),
					{ Position = UDim2.new(s, 0, 1 - (v or 0), 0) }
				)
				tween1:Play()
			...
			HueBase.InputBegan:Connect(function(input)
				if input.UserInputType == Enum.UserInputType.MouseButton1 and not Dragging then
					...
					MC = zzMouse.Move:Connect(function()
						set(math.clamp((zzMouse.Y - HueBase.AbsolutePosition.Y) / HueBase.AbsoluteSize.Y, 0, 1), Cs, Cv)
					end)
			...
			SatBase.InputBegan:Connect(function(input)
				if input.UserInputType == Enum.UserInputType.MouseButton1 and not Dragging then
					...
					MC = zzMouse.Move:Connect(function()
						set(
							Ch,
							math.clamp((zzMouse.X - SatBase.AbsolutePosition.X) / SatBase.AbsoluteSize.X, 0, 1),
							1 - math.clamp((zzMouse.Y - SatBase.AbsolutePosition.Y) / SatBase.AbsoluteSize.Y, 0, 1)
						)
					end)
					IC = input.Changed:Connect(function()
						if input.UserInputState == Enum.UserInputState.End then
	...
	function TabLibrary:NewConfigTab(CustomCredits) --messy xd
		...
		if not isfolder("RealZzHub") then
			makefolder("RealZzHub")
		end
		...
		for _, v in pairs(listfiles(Path)) do
			local PathNames = string.split(v, "\\")
			local Name = PathNames[#PathNames]
			print(Name)
			table.insert(configs, Name)
			ConfigDropdown:AddItem(Name)
		end
		...
		CustomCredits = CustomCredits
			or tostring(game:HttpGet("https://raw.githubusercontent.com/RealZzHub/MainV2/main/Misc/Credits.txt"))
